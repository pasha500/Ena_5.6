...
#include "SceneView.h"
#include "PashaCableStats.h"
#include "DrawDebugHelpers.h"
#include "RayTracingInstance.h"
#include "Kismet/GameplayStatics.h"
#include "GameFramework/Pawn.h"

DEFINE_RENDER_COMMAND_PIPE(PashaCable, ERenderCommandPipeFlags::None);

static TAutoConsoleVariable<int32> CVarRayTracingBetterCableMeshes(
	TEXT("r.RayTracing.Geometry.BetterCable"),
	1,
	TEXT("Include Better Cable (PashaCableComponent) meshes in ray tracing effects (default = 1 (cable meshes enabled in ray tracing))"));

static TAutoConsoleVariable<int32> CVarRayTracingBetterCableMeshesWPO(
	TEXT("r.RayTracing.Geometry.BetterCable.WPO"),
	1,
...
static TAutoConsoleVariable<int32> CVarRayTracingBetterCableMeshesWPOCulling(
	TEXT("r.RayTracing.Geometry.BetterCable.WPO.Culling"),
	1,
	TEXT("Enable culling for WPO evaluation for cable meshes in ray tracing (default = 1 (Culling enabled))"));

static TAutoConsoleVariable<float> CVarRayTracingBCableMeshesWPOCullingRadius(
	TEXT("r.RayTracing.Geometry.BetterCable.WPO.CullingRadius"),
	12000.0f, // 120 m
...
DECLARE_CYCLE_STAT(TEXT("Cable Sim"), STAT_Cable_SimTime, STATGROUP_PashaCablePhysic);
DECLARE_CYCLE_STAT(TEXT("Cable Solve"), STAT_Cable_SolveTime, STATGROUP_PashaCablePhysic);
DECLARE_CYCLE_STAT(TEXT("Cable Collision"), STAT_Cable_CollisionTime, STATGROUP_PashaCablePhysic);
DECLARE_CYCLE_STAT(TEXT("Cable Integrate"), STAT_Cable_IntegrateTime, STATGROUP_PashaCablePhysic);
...
/** Index Buffer */
class FPashaCableIndexBuffer : public FIndexBuffer
{
public:
	virtual void InitRHI(FRHICommandListBase& RHICmdList) override
	{
		const FRHIBufferCreateDesc CreateDesc =
			FRHIBufferCreateDesc::CreateIndex<int32>(TEXT("FCableIndexBuffer"), NumIndices * 2)
...
class FPashaCableSceneProxy final : public FPrimitiveSceneProxy
{
public:
	...
	FPashaCableSceneProxy(UPashaCablePhysic* Component)
		: FPrimitiveSceneProxy(Component)
		, Material(NULL)
		, VertexFactory(GetScene().GetFeatureLevel(), "FPashaCableSceneProxy")
		...
		, TileMaterial(Component->TileMaterial)
	{
		...
		ENQUEUE_RENDER_COMMAND(InitCableResources)(UE::RenderCommandPipe::PashaCable,
...
#if RHI_RAYTRACING
			if (bSupportRayTracing && CVarRayTracingBetterCableMeshes.GetValueOnRenderThread() != 0)
			{
...
private:
	...
	{
		if (CVarRayTracingBetterCableMeshes.GetValueOnRenderThread() == 0)
		{
			return;
		}
		...
		bool bEvaluateWPO = bDynamicRayTracingGeometry && CVarRayTracingBetterCableMeshesWPO.GetValueOnRenderThread() == 1;

		if (bEvaluateWPO && CVarRayTracingBetterCableMeshesWPOCulling.GetValueOnRenderThread() > 0)
		{
			const FVector ViewCenter = Collector.GetReferenceView()->ViewMatrices.GetViewOrigin();
			const FVector MeshCenter = GetBounds().Origin;
			const float CullingRadius = CVarRayTracingBCableMeshesWPOCullingRadius.GetValueOnRenderThread();
...
private:
	...
	FPashaCableIndexBuffer IndexBuffer;
...
UPashaCablePhysic::UPashaCablePhysic(const FObjectInitializer& ObjectInitializer)
...
FPrimitiveSceneProxy* UPashaCablePhysic::CreateSceneProxy()
{
	return new FPashaCableSceneProxy(this);
}

int32 UPashaCablePhysic::GetNumMaterials() const
{
	return 1;
}

/* |||||||||||||||||||||||||||||||||||||  C A B L E   I N I T I A L I Z A T I O N   |||||||||||||||||||||||||||||||||||*/
void UPashaCablePhysic::OnRegister()
{
	Super::OnRegister();
...
void UPashaCablePhysic::RefreshParticlesIsFree()
{
	TArray<int> NotFreePoints;
	for (int i = 0; i < AttachToArray.Num(); i++)
	{
...
void UPashaCablePhysic::VerletIntegrate(float InSubstepTime, const FVector& Gravity)
{
...
void UPashaCablePhysic::SolveConstraints()
{
...
bool UPashaCablePhysic::CheckSubSegmentCollision(FCableSingleParticle& ParticleA, FCableSingleParticle& ParticleB, float Alpha)
{
	UWorld* World = GetWorld();
	...
	if (World && GetCollisionEnabled() != ECollisionEnabled::NoCollision)
	{
		...
		if (bHit)
		{
			// W przypadku kolizji, wykonujemy korekt?na ParticleA i ParticleB, proporcjonalnie do Alpha.
...
void UPashaCablePhysic::PerformCableCollision()
{
...
void UPashaCablePhysic::PerformSubstep(float InSubstepTime, const FVector& Gravity)
{
...
void UPashaCablePhysic::SetAttachIndexToComponent(USceneComponent* Component, FName SocketName, int ParticleIndex)
{
...
void UPashaCablePhysic::SetAttachIndexToActor(AActor* Actor, FName ComponentProperty, FName SocketName, int ParticleIndex)
{
	int PIndex = FMath::Clamp<int>(ParticleIndex, 1, Particles.Num() - 1);
...
TArray<AActor*> UPashaCablePhysic::GetAttachedActors() const
{
...
TArray<FCableAttachPoint> UPashaCablePhysic::GetAttachStructure() const
{
	return AttachToArray;
}

bool UPashaCablePhysic::DetachParticleFrom(int ParticleIndex)
{
	int PIndex = FMath::Clamp<int>(ParticleIndex, 1, Particles.Num() - 1);
	for (int i = 0; i < AttachToArray.Num(); i++)
	{
...
void UPashaCablePhysic::GetCableParticleLocations(TArray<FVector>& Locations) const
{
	Locations.Empty();
...
TArray<FExposedCableParticle> UPashaCablePhysic::GetCableParticlesStructure() const
{
...
TArray<float> UPashaCablePhysic::GetCableInitSegmentsLength() const
{
	return SegmentsLength;
}

TArray<float> UPashaCablePhysic::GetCableCurrentSegmentsLength() const
{
	return CurrentSegmentsLength;
}

TArray<int> UPashaCablePhysic::GetIndicesOfAttachedPoints() const
{
...
bool UPashaCablePhysic::BreakAttachIfSegmentIsStretched(int& ParticleIndex, float StretchTolerance)
{
...
bool UPashaCablePhysic::GetAnyPointIsColliding(int RangeMin, int RangeMax) const
{
...
void UPashaCablePhysic::UseForceToTightenTheRope(int EndIndex, int StartIndex, float ForceStrenght, bool ApplyDistanceForce)
{
...
bool UPashaCablePhysic::ReduceForceForParticles(int StartIndex, int EndIndex, float ReductionStrenght, float dt)
{
...
void UPashaCablePhysic::UpdateSubstepTime(float NewSubstepTime, bool FixVelocity)
{
...
void UPashaCablePhysic::AddImpulseToParticle(FVector Impulse, int ParticleIndex, bool UseDistributedForce, int UnfoldingRange)
{
	...
	// Oblicz przybli�on?pr?ko�� cz�stki
	FVector Velocity = Particle.Position - Particle.OldPosition;
	// Dodaj impuls do pr?ko�ci cz�stki
	Velocity += Impulse;
...
bool UPashaCablePhysic::ApplyConstForceToParticle(FVector Force, int ParticleIndex, bool ExpadLinear, int ExpandLenght, float ExpandMinRange)
{
...
bool UPashaCablePhysic::GetCableSpaceLenghtForSwinging(int& ParticlesCount, FExposedCableParticle& FirstParticle, FExposedCableParticle& LastParticle,
...
void UPashaCablePhysic::SetParticleLocation(FVector Location, int ParticleIndex)
{
	int SafeIndex = FMath::Clamp<int>(ParticleIndex, 0, Particles.Num() - 1);
	Particles[SafeIndex].TargetLocation = Location;
}

void UPashaCablePhysic::UpdateCableLength(float NewLenght, bool RefreshCableInitValues)
{
	if (NewLenght > 0)
	{
...
TArray<float> UPashaCablePhysic::GetStretchTolleranceValuePerSegment()
{
...
void UPashaCablePhysic::GetStartPosition(FVector& OutStartPosition)
{
	OutStartPosition = GetComponentLocation();
}

// Znajdz pozycje komponentu, do ktorego ma byc dolaczona lina
FVector UPashaCablePhysic::GetAttachPosition(FCableAttachPoint& in)
{
...
void UPashaCablePhysic::RefreshLodState()
{
...
FExposedCableParticle UPashaCablePhysic::PrivateToExposedStructure(FCableSingleParticle& InStruct)
{
...
void UPashaCablePhysic::OnVisibilityChanged()
{
...
void UPashaCablePhysic::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UPashaCablePhysic::CreateRenderState_Concurrent(FRegisterComponentContext* Context)
{
...
void UPashaCablePhysic::ApplyWorldOffset(const FVector& InOffset, bool bWorldShift)
{
...
void UPashaCablePhysic::SendRenderDynamicData_Concurrent()
{
	if (SceneProxy)
	{
		...
		FPashaCableSceneProxy* CableSceneProxy = (FPashaCableSceneProxy*)SceneProxy;
...
FBoxSphereBounds UPashaCablePhysic::CalcBounds(const FTransform& LocalToWorld) const
{
...
void UPashaCablePhysic::QuerySupportedSockets(TArray<FComponentSocketDescription>& OutSockets) const
{
...
FTransform UPashaCablePhysic::GetSocketTransform(FName InSocketName, ERelativeTransformSpace TransformSpace) const
{
...
bool UPashaCablePhysic::HasAnySockets() const
{
	return (Particles.Num() >= 2);
}

bool UPashaCablePhysic::DoesSocketExist(FName InSocketName) const
{
	for (int i = 0; i < AttachToArray.Num(); i++)
	{
