...
void UJacobMotionWarpingComponent::WarpingUpdate(FName X, FName Y, FName Z, FName R, bool UseUpAxis, bool WithSweep, bool WarpRotation, float StrengthAlpha, FName ConstAlphaCurve)
{
	...
	{
		SavedWarpActorRot = UKismetMathLibrary::NormalizedDeltaRotator(FRotator(0.0, WarpingTarget->GetActorRotation().Yaw,0.0), RotationOffset);
	}
	//Step 2)
		...
		DeltaRotation = UKismetMathLibrary::NormalizedDeltaRotator(
			UKismetMathLibrary::NormalizedDeltaRotator(
				UKismetMathLibrary::MakeRotFromX(
					UKismetMathLibrary::GetForwardVector(SavedWarpActorRot) * -1.0), FRotator(0, GetCurveValueByIndex(3), 0)), FRotator(0.0, CharacterC->GetActorRotation().Yaw,0.0));
