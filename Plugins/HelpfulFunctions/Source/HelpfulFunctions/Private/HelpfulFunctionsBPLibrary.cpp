...
FVector UHelpfulFunctionsBPLibrary::GetPlayerCapsuleStartLocation(const UObject* WorldContextObject, const ACharacter* TargetChar)
{
	...
	FVector Loc = TargetChar->GetActorLocation();;
	...
	FVector preLocation, FVector preNormal, bool IsFirst, bool ForLeftSide, int DebugIndex, ECollisionChannel TraceChannel)
{
	...
	const bool CapsuleReturnHit = UKismetSystemLibrary::CapsuleTraceSingle(TargetChar, FirstTraceOrigin, FirstTraceOrigin + (ForwardDirection * ForwardScale), 10.0, 
			   CapTraceHeight, MyTraceQuery, false, ToIgnore, TraceType, CapsuleHit, true, FLinearColor::Gray, FLinearColor::Blue, 0.2f);
	...
	bool IsFirst, bool ForLeftSide, int DebugIndex, ECollisionChannel TraceChannel, float NormalFindOffset)
{
	...
	const bool CapsuleReturnHit = UKismetSystemLibrary::CapsuleTraceSingle(TargetChar, FirstTraceOrigin, FirstTraceOrigin + (ForwardDirection * ForwardScale), 10.0,
		CapTraceHeight, MyTraceQuery, false, ToIgnore, TraceType, CapsuleHit, true, FLinearColor::Gray, FLinearColor::Blue, 0.06f);
	...
	float& ReturnCrouchingPlayRate, float Speed, float DeltaX, const float AnimatedWalkSpeed, const float AnimatedRunSpeed, const float AnimatedSprintSpeed, const float AnimatedCrouchSpeed, FName WeightCurveName, bool ReturnOnlyDirection)
{
	...
	if (IsValid(AnimBP->TryGetPawnOwner()) == false)
	{
		...
		ReturnStandingPlayRate = 0.0; ReturnCrouchingPlayRate = 0.0;
		return;
	}
	...
	int Gait, int RotationMode, int MovementDirection, FRotator AimingRotation, FVector Velocity, UCurveVector* YawOffset_FB, UCurveVector* YawOffset_LR)
{
	...
	if (IsValid(AnimBP->TryGetPawnOwner()) == false)
	{
		ReturnDirection = 0; FYaw = 0.0; BYaw = 0.0; LYaw = 0.0; RYaw = 0.0; return;
	}
...
float UHelpfulFunctionsBPLibrary::EstimateTravelTime(float Distance, float MaxWalkSpeed, float MaxAcceleration, float BrakingDeceleration, bool bUseSeparateBrakingFriction, 
	float BrakingFriction, float GroundFriction, float BrakingFrictionFactor, bool bIncludeAcceleration, bool bIncludeDeceleration)
{
	if (Distance <= 0.f || MaxWalkSpeed <= 0.f)
	{
		return 0.f;
	}

	// Obliczenie tarcia (na oko)
	...
	float DecelTerm = ActualBrakingFriction * MaxWalkSpeed + BrakingDeceleration;

	// Przyspieszanie
	float TimeToMaxSpeed = MaxWalkSpeed / MaxAcceleration;
	float DistanceAccel = bIncludeAcceleration ? 0.5f * MaxAcceleration * FMath::Square(TimeToMaxSpeed) : 0.f;
	float TimeAccel = bIncludeAcceleration ? TimeToMaxSpeed : 0.f;

	// Hamowanie
	float TimeToStop = bIncludeDeceleration && DecelTerm > 0.f ? MaxWalkSpeed / DecelTerm : 0.f;
	float DistanceDecel = bIncludeDeceleration ? 0.5f * DecelTerm * FMath::Square(TimeToStop) : 0.f;

	// Ruch jednostajny
	float DistanceFlat = Distance - (DistanceAccel + DistanceDecel);

	if (DistanceFlat < 0.f)
	{
		float EffectiveAccel = bIncludeAcceleration ? MaxAcceleration : TNumericLimits<float>::Max();
		float EffectiveDecel = bIncludeDeceleration ? DecelTerm : TNumericLimits<float>::Max();

		float Denom = EffectiveAccel * EffectiveDecel / (EffectiveAccel + EffectiveDecel);
		if (Denom > 0.f)
		{
			return FMath::Sqrt(2.f * Distance / Denom);
		}
		return 0.f;
	}

	float TimeFlat = DistanceFlat / MaxWalkSpeed;
	return TimeAccel + TimeFlat + TimeToStop;
}
	...
	int Accuracy, bool UseNormalFromFirstTrace, float ConstForwardOffset, FVector RightOffset, int DebugIndex)
{
	...
	FVector TStart = Character->GetActorLocation() + RightOffset + FVector(0, 0, 15.0);
	...
	//if (SphereHitValid == false) { ValidPoint = false; return; }
	const bool SphereHitValid = UKismetSystemLibrary::CapsuleTraceSingle(Character, TStart, TEnd, 12, Character->GetCapsuleComponent()->GetScaledCapsuleHalfHeight() - 15, Channel, false, ActorsToIgnore, 
		TraceType, SphereHitResult, true, FLinearColor(0.16, 0, 0.01, 1), FLinearColor::Red, 0.2);

	float MapIndex;
	bool RequiredTrace = false;
	...
	for (int i = 0; i < Accuracy; i++)
	{
		...
		if (i < UKismetMathLibrary::Round((float)Accuracy/4.0))
		{
			if (Trace2Valid == true)
			{
				RequiredTrace = true;
		...
		if (Trace2Valid == true)
		{
			if (Character->GetCharacterMovement()->IsWalkable(Trace2Hit) == false)
			{
	...
	if (TraceCount - HitCount > 2 || RequiredTrace == false) { ValidPoint = false; return; }
	...
	ReturnCondition = ReturnCondition && UKismetMathLibrary::NearlyEqual_FloatFloat(MinFromDistance, AverageDistances / DistancesArray.Num(), 40.0);
	if (!ReturnCondition == false)
	{

		//GEngine->AddOnScreenDebugMessage(-1, 0.1, FColor::Orange, TEXT("FindNearPointToWallSurface - NearlyEqualFailed"));
		//GEngine->AddOnScreenDebugMessage(-1, 0.1, FColor::Orange, FVector(MinFromDistance, AverageDistances / DistancesArray.Num(), 0).ToString());
		//for (int ii = 0; ii < DistancesArray.Num(); ii++)
		//{
		//	GEngine->AddOnScreenDebugMessage(-1, 0.1, FColor::Orange, FVector(ii * 1.0, DistancesArray[ii], AverageDistances).ToString());
		//}
		ValidPoint = true;
...
float UHelpfulFunctionsBPLibrary::GetCurveValueAtTime(UObject* WorldContextObject, UAnimSequence* AnimSequence, FName CurveName, float Time)
{
	...
	double t1 = Time;
	const FAnimExtractContext ExtractContext(t1, false);
	return AnimSequence->EvaluateCurveData(CurveName, ExtractContext);
}
...
void UHelpfulFunctionsBPLibrary::TryMarkPackageDirty(UObject* WorldContextObject, AActor* Target)
{
	if (Target)
	{
	...
	{
		WorldContextObject->MarkPackageDirty();
	}
}
