...
void UCpp_DynamicClimbingComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	// Safety recovery for cases where climb-exit notifies are skipped.
	if (IsValid(CharacterC) && IsValid(CharacterC->GetCharacterMovement()))
	{
		const bool bGrounded = CharacterC->GetCharacterMovement()->IsMovingOnGround();
		if (!IsClimbingC && ActionC == CMC_ActionTypeC::None)
		{
			if (StartNarrowFloorMovementC && bGrounded)
			{
				StartNarrowFloorMovementC = false;
			}
			ResizeCapsuleToDefaultC(120.0f);
		}
	}
}
	...
	float RightOffsetScale, bool UseWallCondition, bool InverseTracing)
{
	...
	if (bRequireModifyVolumeForLedgeDetection && !IsValid(CurrentModifyVolume))
	{
		Valid = false;
		FirstNotValid = true;
		return;
...
void UCpp_DynamicClimbingComponent::CheckCanStartCornerC_Implementation(bool& DetectedCorner, bool& OuterType, FCMC_LedgeC& TargetLedgeStruct, bool Valid, bool InputLock)
{
	...
	TOrigin + (CharacterC->GetActorRightVector() * AxisValuesC.Y * 50.0), 18, 38, Channel, 
